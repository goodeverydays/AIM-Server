...
AIM-Chat 提供基于 **Qt 6 + Qt Quick/QML** 的 Windows 桌面客户端。界面层使用 QML 构建，C++ 负责网络通信、协议编解码、会话状态和本地数据访问；客户端通过 TCP 长连接接入 IMServer，并通过 Protobuf 与服务端交换结构化消息。

**Windows x64 体验版：** [下载 IMClient 1.0.0](https://github.com/goodeverydays/AIM-Chat/releases/tag/v1.0.0)

> 当前 `v1.0.0` 定位为作品集预发行版本，支持 Windows 10/11 x64。在线功能依赖演示服务器；安装包尚未进行商业代码签名，通信链路尚未启用 TLS，请不要过度使用其中的智能助手，当前客户端仅为展示服务端功能而诞生。

### 登录与注册

<table>
  <tr>
    <td width="42%"><img src="docs/images/imclient-login.png" alt="AIM-Chat 登录界面"></td>
    <td width="58%"><img src="docs/images/imclient-register.png" alt="AIM-Chat 注册界面"></td>
  </tr>
  <tr>
    <td align="center">账号密码登录</td>
    <td align="center">邮箱验证码注册</td>
  </tr>
</table>

- **登录流程**：输入用户名和密码后建立 TCP 会话，登录成功进入聊天主界面；密码输入框支持显隐切换。
- **注册流程**：填写用户名、昵称和邮箱，通过邮件验证码完成身份确认，并进行两次密码一致性校验。
- **账号能力**：登录后可维护昵称、头像和密码，客户端根据服务端推送同步好友状态与会话数据。

### 聊天与智能助手

<p align="center">
  <img src="docs/images/imclient-main.png" alt="AIM-Chat 聊天主界面" width="900">
</p>

主界面采用会话导航与聊天工作区布局：左侧集中展示当前账号、在线状态、好友搜索、好友/群组列表和最近会话；右侧承载消息记录、输入区及当前聊天对象信息。

- **即时通信**：支持好友申请、在线状态、单聊、群聊、历史消息和未读状态。
- **本地存储**：使用 SQLite 保存本地聊天记录，减少重复拉取并支持会话恢复。
- **媒体消息**：支持头像、语音和视频的上传、下载及播放。
- **智能助手**：提供普通 AI 对话、聊天总结、回复建议和基于历史消息的 RAG 记忆问答。
- **连接管理**：实现心跳检测、请求超时与断线重连，提升公网环境下的可用性。

| 客户端层次 | 实现方式 |
|---|---|
| 界面与交互 | Qt Quick / QML，组件化页面与状态绑定 |
| 网络通信 | `QTcpSocket` 长连接、自定义帧协议、Protobuf、Adler32 校验 |
| 会话状态 | C++ 业务层统一管理登录态、好友、群组和消息事件 |
| 本地数据 | SQLite 聊天记录与会话缓存 |
| 在线能力 | IMServer 转发，Go 服务提供邮箱、头像、媒体与 AI/RAG 能力 |
